"""
Counts number of letters used to represent all numbers in a sequence.
"""
...
def num_block_con(num, level):
    ...
    for item in num_string:
        if position_count == 3 and item != '0':
            string += num_alpha_dict[int(item)] + ' ' + 'hundred'
            if num_string[-2:] != '00':
                string += ' and '
            position_count -= 1
        elif position_count == 2 and int(item) != 1:
            string += num_alpha_dict[int(item)*10] + ' '
            position_count -= 1
        elif position_count == 2 and int(item) == 1:
            string += num_alpha_dict[int(item+num_string[-1])]
            break
        else:
            string += num_alpha_dict[int(item)]
            position_count -= 1
    string += ' ' + levels_dict[level]
    return string
...
def num_to_block(num):
    ...
    while num_len >= 3 and num_string:
        word_list.append(num_block_con(num_string[-3:], level))
        num_string = num_string[:-3]
        level += 1
    while num_len < 3 and num_string:
        word_list.append(num_block_con(num_string[-num_len:], level))
